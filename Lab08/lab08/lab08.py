...
def in_range2(n):
    ...
    if not (n >=1 and n <= 100):
        raise ValueError("Number not found within range")
...
def bound_checker(x_dimension, y_dimension, x, y):
    ...
    if 0 <= x < x_dimension and 0 <= y < y_dimension:
        ...
        raise IndexError("Numbers not within range")
...
def handle_user_input():
    ...
    try:
        ...
        raise ValueError(f"Invalid age: {user_input} contains non-numerical characters")
