...
def height(t):
    ...
    if t.is_leaf():
        ...
        total = 1
        for branch in t.branches:
            if height(branch) > highestBranch:
                highestBranch = height(branch)
    return total + highestBranch
...
def max_path_sum(t):
    ...
    if t.is_leaf():
        ...
        total = t.label
        for branch in t.branches:
            if max_path_sum(branch) > highestBranch:
                highestBranch = max_path_sum(branch)
    return total + highestBranch
...
class Tree:
    ...
    def __str__(self):
        ...
        return '\n'.join(indented(self))
sproul = Tree('roots', [Tree('branch1', [Tree('leaf'), Tree('berry')]), Tree('branch2')])
print(sproul.__str__())
print(sproul.__repr__())
