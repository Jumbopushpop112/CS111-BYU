...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'C ?S ?111R?', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b[0-2]?\d:[0-5]\dA?P?M?\b', text)
...
def most_common_code(text):
    ...
    """
    """*** YOUR CODE HERE ***"""
    patternList = area_codes(text)
    return max(patternList, key=patternList.count)
