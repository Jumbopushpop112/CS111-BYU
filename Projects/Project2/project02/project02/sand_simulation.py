...
def add_object(grid, object_type, x, y):
    ...
    """
    if not grid.array[y][x] == None:
        return
    else:
        curObject = object_type(grid,x,y)
        all_grid_objects.append(curObject)
        grid.set(x,y,curObject)
...
def remove_object(grid, x, y):
    ...
    """
    if grid.array[y][x] == None:
        return
    else:
        curObject = grid.get(x,y)
        grid.array[y][x] = None
        all_grid_objects.remove(curObject)
...
def do_whole_grid():
    ...
    """*** YOUR CODE HERE ***"""
    for curObject in all_grid_objects:
        if isinstance(curObject, Bubble):
            curObject.move()
    all_grid_objects.reverse()
    for curObject in all_grid_objects:
        if isinstance(curObject, Sand):
            curObject.move()
    all_grid_objects.reverse()
